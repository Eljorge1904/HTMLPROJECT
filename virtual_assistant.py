...
while True:
    ...
    if status:
        if 'estas ahi' in rec:
            ...
            webbrowser.open('https://www.netflix.com/mx/login')
            speak('abriendo netflix')
        
        elif 'escuela' in rec:
            webbrowser.open('https://magno.spilbit.com/login/index.php')
            speak('abriendo escuela')
        
        elif 'amor' in rec:
            speak('iniciando')
            webbrowser.open('https://www.youtube.com/watch?v=waU75jdUnYw&list=PL68rPXU-lZhSwo4IgUqRBC1ZNa_3XFk6_&ab_channel=TheWeekndVEVO')
        
        elif 'busca' in rec:
            rec = rec.replace(f"busca ", "")
            speak('buscando')
            webbrowser.open(rec)
            print(rec)
        ...
        elif 'descansa' in rec or 'apagate' or 'basta' or 'para':
            speak("Saliendo...")
            break
